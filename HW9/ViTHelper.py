...
class MasterEncoder(nn.Module):
    def __init__(self, max_seq_length, embedding_size, how_many_basic_encoders, num_atten_heads, task=1):
        super().__init__()
        self.max_seq_length = max_seq_length
        self.basic_encoder_arr = nn.ModuleList([BasicEncoder(
            max_seq_length, embedding_size, num_atten_heads, task) for _ in range(how_many_basic_encoders)])  # (A)
...
class BasicEncoder(nn.Module):
    def __init__(self, max_seq_length, embedding_size, num_atten_heads, task=1):
        super().__init__()
        self.max_seq_length = max_seq_length
        ...
        self.self_attention_layer = SelfAttention(
            max_seq_length, embedding_size, num_atten_heads, task)  # (A)
...
class SelfAttention(nn.Module):
    def __init__(self, max_seq_length, embedding_size, num_atten_heads, task=1):
        ...
        if(task==1):
            ...
            self.attention_heads_arr = nn.ModuleList([AttentionHead2(self.max_seq_length,
                                                                self.qkv_size) for _ in range(num_atten_heads)])
...
class AttentionHead2(nn.Module):
    ...
    def forward(self, sentence_portion): 
        ...
        Q = self.WQ(sentence_portion.reshape(batch_size, -1).float()).to(device)
        K = self.WK(sentence_portion.reshape(batch_size, -1).float()).to(device)
        V = self.WV(sentence_portion.reshape(batch_size, -1).float()).to(device)
        ...
        QK_dot_prod = torch.einsum("bqn, bnk -> bqk", Q, rearrange(K, "b n e -> b e n")) # shape change from [16, 17, emb_size / heads] -> [16, emb_size / heads, 17]
        scaling_coeff = 1.0 / torch.sqrt(torch.tensor([self.qkv_size]).float()).to(device)
        return scaling_coeff * torch.einsum("ban, bne -> bae", self.softmax(QK_dot_prod), V)